...
export const Breadcrumbs: React.FC = () => {
  ...
  const calculateBreadcrumbs = () => {
    ...
    if (pathnames[0] === 'productos' || pathnames[0] === 'producto') {
      ...
      // CASO B: Es el Catálogo General (No hay ID o no se encontró producto)
      else {
        const isFiltered = catParam || subParam || qParam;
        crumbs.push({ label: 'Productos', path: isFiltered ? '/productos' : null });
        
        if (catParam) {
          crumbs.push({ label: catParam, path: subParam || qParam ? `/productos?cat=${encodeURIComponent(catParam)}` : null });
        }
        if (subParam) {
          crumbs.push({ label: subParam, path: null });
        }
        if (qParam) {
        ...
        {breadcrumbs.map((crumb, index) => {
          ...
          return (
            <div key={index} className="flex items-center shrink-0">
              {index > 0 && <span className="text-slate-300 shrink-0 mx-1">/</span>}
              
              {!crumb.path || isLast ? (
                <span className="text-[#2553A8] flex items-center gap-1 shrink-0 animate-in fade-in slide-in-from-left-1 duration-300">
                  {crumb.icon && <span className="material-symbols-outlined text-[12px]">{crumb.icon}</span>}
                  {crumb.label}
                </span>
              ) : (
                <a 
