...
export const WhatsAppButton: React.FC = () => {
  ...
  const lines = [
    ...
    },
    {
      name: "Sede Alterna",
      phone: "573174675905", 
      message: "Hola, necesito cotizar un equipo...",
      icon: "storefront",
    ...
    >
      ...
      `}>
        ...
        <div className="bg-[#128c3f] text-white p-4 rounded-t-3xl rounded-b-xl mb-2 text-center">
          <p className="font-bold text-sm">👋 ¡Hola! ¿Con qué sede deseas hablar?</p>
          <p className="text-[10px] opacity-80 mt-1">Solemos responder en minutos</p>
        </div>
          ...
          {lines.map((line, index) => (
            ...
            >
              <div className="size-10 bg-slate-100 rounded-full flex items-center justify-center shrink-0 group-hover:bg-white group-hover:shadow-md transition-all">
                <span className={`material-symbols-outlined ${line.color} text-xl`}>
                  {line.icon}
                </span>
              </div>
              <div className="flex flex-col">
                ...
                <span className={`text-[10px] font-medium flex items-center gap-1 ${isAvailable ? 'text-green-500' : 'text-red-500'}`}>
      ...
      >
        ...
        <span className={`text-3xl transition-transform duration-300 absolute ${isOpen ? 'opacity-0 rotate-90' : 'opacity-100 rotate-0'}`}>
           { <i className="fa-brands fa-whatsapp text-4xl"></i>}
        </span>
        <span className={`material-symbols-outlined text-3xl transition-transform duration-300 absolute ${isOpen ? 'opacity-100 rotate-0' : 'opacity-0 -rotate-90'}`}>
           close
        </span>
        ...
        <div className={`absolute right-full mr-4 bg-white text-slate-700 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wide shadow-lg border border-slate-100 whitespace-nowrap transition-all duration-300 ${isOpen ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0'}`}>
           ¿Necesitas ayuda?
